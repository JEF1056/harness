...
function identifyEntryPoints(root: string): string[] {
    ...
    if (fs.existsSync(packageJsonPath)) {
        try {
            ...
            if (pkg.main) entryPoints.push(path.join(root, pkg.main));
            if (pkg.bin) {
...
function detectLanguage(root: string): string {
    if (fs.existsSync(path.join(root, "package.json"))) return "TypeScript/JavaScript";
    ...
    if (fs.existsSync(path.join(root, "build.gradle")) || fs.existsSync(path.join(root, "pom.xml"))) return "Java/Kotlin";
    return "Unknown";
}
...
function detectBuildSystem(root: string): string {
    if (fs.existsSync(path.join(root, "package.json"))) return "npm/yarn/pnpm";
    if (fs.existsSync(path.join(root, "Makefile"))) return "Make";
    if (fs.existsSync(path.join(root, "build.gradle"))) return "Gradle";
