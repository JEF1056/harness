...
function isTestFile(file: string): boolean {
    ...
    if (parts.some(p => /^(test|tests|__tests__|spec|specs)$/.test(p))) return true;
...
export function isMapStale(workspaceRoot: string): boolean {
    ...
    if (f.recordedCommit === f.gitCommit) return false;
    try {
 ...
 */
export function extractInterfacesSection(content: string): string | null {
    const m = content.match(INTERFACES_SECTION_RE);
    return m ? m[0] : null;
}

function splitModuleBlocks(section: string): Array<{ mod: string; lines: string[] }> {
    const out: Array<{ mod: string; lines: string[] }> = [];
 ...
 * (or "## Freshness").
 */
export function withInterfacesSection(freshDoc: string, preserved: string): string {
