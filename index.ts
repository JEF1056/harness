...
import { CODEBASE_MAP_PROMPT, build_codebase_map, isMapStale, extractEnrichmentNote, extractInterfacesSection, withInterfacesSection } from "./map.js";
...
function acquireWorkspaceLock(agentsDir: string, sessionId: string): { locked: true; lockData: any } | { locked: false; owner: string } {
    try {
...
function releaseWorkspaceLock(agentsDir: string): void {
    try { fs.unlinkSync(path.join(agentsDir, "lock.json")); } catch {}
}

function isLockStale(lockData: any): boolean {
...
function resolveSubagentModel(agentName: string, config: any, harnessConfig?: { models?: Record<string, string> }): string | undefined {
...
function buildMapDigest(workspaceRoot: string): string | null {
    ...
    if (freshMatch) {
        ...
        if (commitLine) lines.push(`- Map freshness: commit ${commitLine[1].trim()}`);
        if (statusLine) lines.push(`- Enrichment: ${statusLine[1].trim()}`);
    }
...
function tryAutoRefreshMap(workspaceRoot: string): boolean {
    const now = Date.now();
    if (now - lastAutoRefresh < 60_000) return false;
    lastAutoRefresh = now;
    try {
        const mapPath = path.join(workspaceRoot, "CODEBASE_MAP.md");
        if (!fs.existsSync(mapPath)) return false;
        if (!isMapStale(workspaceRoot)) return false;
    ...
    const startHeartbeatMonitor = () => {
        ...
        heartbeatInterval = setInterval(() => {
            ...
            for (const agent of agents) {
                ...
                if (agent === 'orchestrator' || agent === 'sentinel' || agent === 'state.json' || agent === 'plans' || agent === 'sessions' || agent === 'skills' || agent === 'lock.json') continue;
    ...
    const startWatcher = () => {
        try {
            ...
            rootWatcher = fs.watch(agentsDir, (eventType, filename) => {
                ...
                if (filename === 'sessions' || filename === 'lock.json') return;
       ...
       tool: {},
        ...
        "command.execute.before": async (cmdInput: any, cmdOutput: any) => {
            ...
            if (command === "harness") {
                ...
                try {
                    ...
                    const saveSessionWarnedAgents = (s: Set<string>) => {
                        try { fs.writeFileSync(sessionWarnedFile, [...s].join(','), 'utf8'); } catch {}
                    };
        ...
        "experimental.chat.system.transform": async (_input: any, output: any) => {
            ...
            if (Array.isArray(output.system)) {
                ...
                const existing = output.system.filter((s: any) => typeof s === "string" && s.length > 0);
                const alreadyInjected = existing.some((s: string) => s.includes(marker));
                if (!alreadyInjected) {
